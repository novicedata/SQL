...
- 비트: 바이트보다 작음 (2진수 저장)
- 비트 연산: 데이터를 비트 단위로 연산
  - 0 or 1로 표현되는 정수 타입만 비트 연산 가능
  - 두 값을 비트 단위로 나열한 뒤 각 자릿수를 비트 연산자로 연산
  - 각 자릿수는 독립적, 다른 자릿수에 영향 X

|연산자|역할|예|
|---|---|---|
|&|and 연산. 두 자리수 모두에서 1인 자리만 1로|5 & 3 = 1. 5는 0101, 3은 0011이라 마지막 자리수만 1. 그래서 0001로 십진수로 바꾸면 1이됨|
|\||or 연산. 두 수 중 하나에서라도 1이면 1로|5 \| 3 = 7(0111).|
|^|xor 연산. 두 수 중 하나에서만 1인 자리만 1로| 5 ^ 3 = 6(0110)|
|~|not 연산. 1을 0으로, 0을 1로 바꾼다|~5 = -2(1111 1010)|
|a << b|left shift 연산. 모든 자리의 수를 왼쪽으로 b번 옮기고 오른쪽에 생긴 빈자리를 0으로채운다|5(0101) << 1 = 10(1010)|
|a >> b|right shift 연산. 모든 자리의 수를 오른쪽으로 b번 옮기고 왼쪽에 생긴 빈자리는 가장 왼쪽의 수와 같은것으로 채운다|5(0101) >> 1 = 2(0010)|
